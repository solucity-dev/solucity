...
import {
  getAdminCertificationsList,
  type AdminCertificationRowList,
  type AdminCertificationsListResp,
} from '../api/adminApi';
...
export default function CertificationsPage() {
  ...
  const [resp, setResp] = useState<AdminCertificationsListResp | null>(null);

  async function load() {
    setError(null);
    setLoading(true);
    try {
      const r = await getAdminCertificationsList({ status });
      setResp(r);
    } catch (e: unknown) {
      const msg =
        e instanceof Error ? e.message : typeof e === 'string' ? e : 'Error al cargar certificaciones';
      setError(msg);
      setResp(null);
    } finally {
      setLoading(false);
  ...
  const items = useMemo<AdminCertificationRowList[]>(() => {
    ...
    <div style={{ padding: 20 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <button
            onClick={() => navigate('/app/certifications?status=ALL')}
            style={{ padding: '8px 12px', borderRadius: 10, border: '1px solid rgba(0,0,0,0.12)' }}
          >
            Todas (debug)
          </button>
        ...
        <div style={{ marginTop: 14 }}>
          ...
          {items.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {items.map((c) => {
                const specialistId = c.specialist?.specialistId ?? null;
                const name = c.specialist?.name ?? '—';
                const email = c.specialist?.email ?? '—';
                const cat = c.category?.name ?? c.category?.slug ?? '—';
                const createdAt = formatDateAR(c.createdAt);
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
                      <div style={{ minWidth: 0 }}>
                        ...
                        <div style={{ marginTop: 10, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                          ...
                          <span style={{ opacity: 0.6, fontSize: 12 }}>ID: {c.id}</span>
                        </div>
                      </div>
